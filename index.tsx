document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        const renderStaticParts = () => {
            ...
            (document.getElementById('search-bar') as HTMLInputElement).placeholder = partData.searchPlaceholder;
        ...
        const timelineSelection = document.getElementById('timeline-selection');
        const timelineContent = document.getElementById('timeline-content');
        const timelineSections = document.querySelectorAll<HTMLElement>('.timeline-section');
        const subtitle = document.getElementById('subtitle');
        const searchBar = document.getElementById('search-bar') as HTMLInputElement;
        const searchResultsContainer = document.getElementById('search-results-container');
        let allDetails = document.querySelectorAll<HTMLDetailsElement>('details');
    
        // --- Breadcrumb Feature Elements ---
        const breadcrumbNav = document.getElementById('breadcrumb-nav');
        let breadcrumbState: { name: string; level: string; id?: string }[] = [{ name: partData.breadcrumbHome, level: 'home' }];
        ...
        const readAloud = async (text: string, lang: string, button: HTMLElement) => {
        ...
        // --- Navigation Logic ---
        const navigateToElement = (targetId: string) => {
            const targetElement = document.getElementById(targetId) as HTMLDetailsElement;
            if (targetElement) {
        ...
        // --- Breadcrumb Logic ---
        const getSummaryName = (summary: HTMLElement | null): string => {
            if (!summary) return 'Details';
            const titleElement = summary.querySelector<HTMLElement>('.summary-title');
            if (titleElement) {
        ...
        const updateAndRenderBreadcrumbs = (newState: { name: string; level: string; id?: string }[]) => {
            breadcrumbState = newState; renderBreadcrumbs();
        };
    
        const updateBreadcrumbsFromElement = (element: HTMLElement) => {
            const path: { name: string; level: string; id?: string }[] = [{ name: partData.breadcrumbHome, level: 'home' }];
            const section = element.closest('.timeline-section');
            if (!section) { updateAndRenderBreadcrumbs(path); return; }
            const sectionCard = document.querySelector<HTMLElement>(`.timeline-card[data-target="${section.id}"]`);
        ...
        breadcrumbNav.addEventListener('click', (event) => {
            const target = (event.target as HTMLElement).closest<HTMLElement>('.breadcrumb-item');
            ...
            if (targetCrumb.level === 'home') {
                ...
                section?.querySelectorAll<HTMLElement>('.dynasty-details, .event-details').forEach(d => d.classList.remove('hidden'));
        ...
        const buildSearchIndex = () => {
            ...
            allDetails.forEach((details, index) => {
                ...
                const summaryElement = details.querySelector<HTMLElement>(':scope > summary');
        ...
        searchResultsContainer.addEventListener('click', (event) => {
            const targetItem = (event.target as HTMLElement).closest<HTMLElement>('.search-result-item');
        ...
        timelineSelection.addEventListener('click', (event) => {
            const target = (event.target as HTMLElement).closest<HTMLElement>('.timeline-card');
            if (!target) return;
        ...
        const applyKingColorCoding = () => {
            document.querySelectorAll<HTMLElement>('.dynasty-details').forEach(dynasty => {
                const contentPanel = dynasty.querySelector<HTMLElement>(':scope > .content-panel');
                if (!contentPanel) return;
                const kings = contentPanel.querySelectorAll<HTMLElement>(':scope > .king-details');
        ...
        const setupAccordionLogic = () => {
            ...
            const setPanelState = (panel: HTMLElement, open: boolean) => {
                if (open) {
            ...
            allDetails.forEach((details, i) => {
                const summary = details.querySelector(':scope > summary'); const contentPanel = details.querySelector<HTMLElement>(':scope > .content-panel');
                ...
                details.addEventListener('toggle', (event) => {
                    ...
                    if (target.matches('.dynasty-details, .event-details')) {
                        ...
                        if (parentSection) {
                            parentSection.querySelectorAll<HTMLDetailsElement>('.dynasty-details, .event-details').forEach(other => {
                    ...
                    setTimeout(() => {
                        ...
                        while (parent) {
                            const parentContentPanel = parent.querySelector<HTMLElement>(':scope > .content-panel');
        ...
        // --- Visual Timeline Bar ---
        const createVisualTimelines = (data: any) => {
            if (!data || Object.keys(data).length === 0) return;
            Object.entries(data).forEach(([sectionId, sectionData]: [string, any]) => {
                ...
                sectionData.dynasties.forEach((dynasty: any) => {
        ...
        // --- Inline Glossary ---
        const initializeGlossary = (data: any) => {
            ...
            let popover: HTMLElement | null = null; let lastFocusedTerm: HTMLElement | null = null;
            const removePopover = () => {
                if (synth.speaking) synth.cancel();
                if (popover) { popover.remove(); popover = null; }
                if (lastFocusedTerm) { lastFocusedTerm.focus(); lastFocusedTerm = null; }
            };
            const handleTruncation = (popoverEl: HTMLElement) => {
                const existingBtn = popoverEl.querySelector('.glossary-learn-more'); if (existingBtn) existingBtn.remove();
                const visibleContent = popoverEl.querySelector<HTMLElement>('.glossary-popover-content:not(.hidden)'); if (!visibleContent) return;
            ...
            const handlePopoverKeyDown = (event: KeyboardEvent) => {
                ...
                if (event.key === 'Tab' && popover) { event.preventDefault(); popover.querySelector<HTMLButtonElement>('.glossary-popover-close')?.focus(); }
            };
            const createPopover = (term: string, target: HTMLElement) => {
                ...
                const closeButton = popover.querySelector<HTMLButtonElement>('.glossary-popover-close');
                const toggleBtn = popover.querySelector<HTMLButtonElement>('.language-toggle');
                const readBtn = popover.querySelector<HTMLButtonElement>('.read-aloud-btn');
                closeButton?.focus(); closeButton?.addEventListener('click', removePopover);
                readBtn?.addEventListener('click', (e) => {
                    e.stopPropagation();
                    const visibleTitle = popover.querySelector<HTMLElement>('.glossary-popover-title:not(.hidden)'); const visibleContent = popover.querySelector<HTMLElement>('.glossary-popover-content:not(.hidden)'); if (!visibleTitle || !visibleContent) return;
        ...
        // --- Connections Hub ---
        const createConnectionsHubs = (data: any) => {
            if (!data || Object.keys(data).length === 0) return;
            Object.entries(data).forEach(([detailsId, connections]: [string, any]) => {
            ...
            document.getElementById('timeline-content').addEventListener('click', (event) => {
                const target = (event.target as HTMLElement).closest<HTMLElement>('.connection-tag');
